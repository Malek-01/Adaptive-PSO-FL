...
class GlobalBestPSO_XY(SwarmOptimizer):
    def __init__(
        self,
        n_particles,
        dimensions,
        ...
        """
        super(GlobalBestPSO_XY, self).__init__(
...
#---------------------------------------------------------------------------------------------------
#-----------------------------------Start------------------------------------------------------------
        # Initialisez previous_current_cost
        previous_current_cost = np.full(self.swarm_size[0], np.inf)
        X = []
        Y = []
        cost_history = []
#-----------------------------------------End-------------------------------------------------------        
...
#---------------------------------------------------------------------------------------------------  
#----------------------------------------Start------------------------------------------------------  
        # value y for each iteration    
            y = 0
        # values of c1 and c2 of the previous iteration 
            c1 = self.options['c1']
            c2 = self.options['c2']
        # values of updating c1 and c2 for each strategy
            upVal = ((1, -1), (0.2, -0.2), (0.2, 0.2), (-1, 1))
        # set the value of gbest to inifinity
            gbest = np.inf
        # Loop on all strategies    
            for j in range(4):
                self.options['c1'] = max(0.1, c1 + upVal[j][0])
                self.options['c2'] = max(0.1, c2 + upVal[j][1])
            # Apply the resolution
                self.swarm.current_cost = compute_objective_function(self.swarm, objective_func, pool=pool, **kwargs)
                self.swarm.pbest_pos, self.swarm.pbest_cost = compute_pbest(self.swarm)

                best_cost_yet_found = self.swarm.best_cost
                self.swarm.best_pos, best_cost = self.top.compute_gbest(self.swarm)
            # Find the best strategy and store it in Y
                if best_cost < gbest:
                    gbest = best_cost
                    y = j+1
            Y.append(y)
            self.swarm.best_cost = gbest

        # Check if there is an improvment and fill up the X
            diff_cost = self.swarm.current_cost - previous_current_cost
            for particle_idx, diff in enumerate(diff_cost):
                if diff > 0.2:
                    ...
                    X.append(-1)
            previous_current_cost = self.swarm.current_cost
