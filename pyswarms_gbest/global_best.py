...
class GlobalBestPSO2(SwarmOptimizer):
    def __init__(
        self,
        n_particles,
        dimensions,
        ...
        """
        super(GlobalBestPSO2, self).__init__(
...
#--------------------------------------------------------------------------------------------------------
            ...
            model_path = os.path.join(script_dir, 'svm_model2.pkl')
            loaded_svm_model = joblib.load(model_path)
            # loaded_svm_model = joblib.load('C:/PSO/benchmarck/pyswarms_gbest/svm_model2.pkl')
            matrix = [list_R[i:i+10] for i in range(0, len(list_R), 10)]
            y = loaded_svm_model.predict(matrix)
